class Library:
    ...
    def library_info(self):  # Prints all information stored in library
        ...
        print("Number of available books: ", len(self.books)-1)
        print("Number of registered staff: ", len(self.staff)-1)
    ...
    def library_info_fileWrite(self, file_name):  # Writes the library info, to a file
        ...
        print("Number of available books: ", (len(self.books)-1), file=infile)
        print("Number of registered staff: ", (len(self.staff)-1), file=infile)
    ...
    def book_info_fileWrite(self, file_name):  # Writes all available books info to a file
        ...
        for i in range(1, len(self.books)):
            print("-",i,".", file=infile)
            print("Book Title: ", self.books[i].get_title(), "   -   ", "Genre: ", self.books[i].get_genre(), "   -   ",
                  "Tracking code: ",
...
class Student(Person):
    ...
    def issued_book(self):  # Prints info of all books issued by student
        ...
        for i in range(0, len(self.issuedBooks)):
            print(self.issuedBooks[i].get_title())
...
class Author(Person):
    ...
    def portfolio(self, libaray):
        ...
        for i in range(1, len(bok_list)):
            print(i , ".", bok_list[i].get_title(), " by ", bok_list[i].get_author().get_name())
...
def sample_books():  # Creates sample books

    # Sample Books - 10

    auth1 = Author("F. Scott Fitzgerald")
    book1 = Book("The Great Gatsby","Novel, Fiction, Tragedy",auth1)
    book2 = Book("Tender Is the Night","Novel, Roman à clef, Fictional Autobiography",auth1)
    book3 = Book("This Side of Paradise","Novel, Fiction, Romance novel",auth1)

    auth2 = Author("Stephen King","stephen_king1@gmail.com")
    book4 = Book("The Shining"," Novel, Horror fiction,Psychological horror",auth2)
    book5 = Book("Pet Sematary","Horror fiction",auth2)
    book6 = Book("Doctor Sleep","Horror fiction",auth2)
    book7 = Book("The Green Mile","Novel, Horror fiction",auth2)

    book8 = Book("Long Walk to FreedomBook","Autobiography",Author("Nelson Mandela"))
    book9 = Book("Mein Kampf","Biography",Author("Adolf Hitler","hitler_adolf@yahoo.com"))
    book10 = Book("The Ingenious Gentleman Don Quixote of La Mancha","Parody, Satire, Fiction",Author("Miguel de Cervantes"))

    return book1,book2,book3,book4,book5,book6,book7,book8,book9,book10

def sample_librarian():  # Creates sample librarians

    #  Sample Librarians - 3

    librarian1 = Librarian("Jonathan Joestar",1234,"jonathan@hotmail.com")
    librarian2 = Librarian("Cornelia Britannia",5564,"corn@gmail.com")
    librarian3 = Librarian("Edward Elric",6924,"elric@gmail.com")
    return  librarian1,librarian2,librarian3

def sample_students():  # Creates sample students

    #  Sample students 5

    student1 = Student("Luke Skywalker",1214,"luke@gmail.com")
    student2 = Student("Anakin Skywalker",8759,"anni@yahoo.com")
    student3 = Student("James",7000,"bond@gmail.com")
    student4 = Student("Thanos",5050,"snap@hotmail.com")
    student5 = Student("Gandalf graybeard",9587,"fireworks@gmail.com")
...
def main_loop():
    ...
    head_lib = Librarian("Head Librarian", 1000, "DoNotRemove@gmail.com")
    main_library.register_staff(head_lib)

    test_auth = Author("TEST")
    test_main_book = Book("Main_test_book", "Do_not_remove", test_auth)
    main_library.add_books(test_main_book)

    def add_sample_info():  # Adds all the sample info created previously to the library

        bookss = list(sample_books())
        for i in range(0,len(bookss)):
            main_library.add_books(bookss[i])

        libbs = list(sample_librarian())
        for i in range(0,len(libbs)):
            main_library.register_staff(libbs[i])

        stuus = list(sample_students())
        for i in range(0,len(stuus)):
            main_library.register_student(stuus[i])


    enter_sample_info = input("Would you like to setup pre-made books,students and librarians into the library? (Yes/No) : ")
    if enter_sample_info == "Yes" or enter_sample_info == "YES" or enter_sample_info == "yes" or enter_sample_info == "y" or enter_sample_info == "1":
    ...
    while run_check != 0:
        ...
        if run == "2":
            ...
            if what_action == "1":
                ...
                for i in range(0, how_many):
                    info = input("-Please enter Name, ID, Email: -> (Name/ID/Email)-\n: ")
                    ...
                    if info_list[1] not in iden_li:
                        ...
                        print("There already exists a Librarian with the ID,", info_list[1],
                              ". Please try registering again.")

            if what_action == "2":
                rem_name = input("-Enter name and ID of Librarian to remove: -> (Name/ID)-\n")
        ...
        if run == "7":
            ...
            for i in range(0, int(h_books)):
                author_info = input("-Please enter the authors Name ~AND/OR~ Email -> (Name/Email)\n : ")
        ...
        if run == "14":
            ...
            for i in range(0, len(stu_list14)):
                if stu_list14[i].get_name() == stu_name14_lst[0] and stu_list14[i].get_id() == int(stu_name14_lst[1]):
                    chek = 1
                    st = stu_list14[i]
            if chek == 1:
                st.issued_book()
